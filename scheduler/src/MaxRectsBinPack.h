...
public:
	...
	MaxRectsBinPack(int width, int height, bool allowFlip, int bin);

	/// (Re)initializes the packer to an empty bin of width x height units. Call whenever
	/// you need to restart with a new bin.
	void Init(int width, int height, bool allowFlip, int bin);
	...
	std::vector<Rect> Insert(int width, int height, FreeRectChoiceHeuristic method, bool isOptimized, int app, int input_file, int min_node);
...
private:
	...
	std::vector<Rect> FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX, bool isOptimized, int app, int input_file, int min_node) const;
	//std::vector<Rect> FindPositionForNewNodeBestShortSideFit(int width, int height, int &bestShortSideFit, int &bestLongSideFit) const;
