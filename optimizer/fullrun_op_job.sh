...
function optimize_main()
{
	...
	if [ "$MAX_NODE" != "0" ]
 	then
		...
		CONSTRAINT_FILE=constraint_$(uuidgen).csv
		echo "VMcount,$MIN_NODE,$MAX_NODE,1,int" >> optimizer/$CONSTRAINT_FILE 
		echo "VMtype,$TYPE,$TYPE,1,int" >> optimizer/$CONSTRAINT_FILE 
		...
		out=$(curl -X GET "http://0.0.0.0:9000" -d "${job_type},optimizer/optimize_case/${job_input}_input_${CASE}.csv,optimizer/${CONSTRAINT_FILE},${DEADLINE}" 2> /dev/null)
