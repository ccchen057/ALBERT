...
HIBENCH_DIR=~/HiBench
WORKLOAD_DIR=${HIBENCH_DIR}/bin/workloads/micro/terasort
LOG_OUTPUT=~/hadoop-log/hadoop.log
#LOG_OUTPUT=/dev/null
INPUTDIR_RANGE=4
COMMENT="terasort tring"
...
for i in $jobid ; do
	...
	echoProgress "Get Ceilometer log ($i)"
		...
		echo $hadoop_log $metric_log | jq -s add > data.tmp
		
	if [ $?=="0" ] ; then echo "Done"; fi

	echoProgress "Generate metadata ($i)"
		info=$(bash genInfo.sh "data.tmp" "$COMMENT")
	if [ $?=="0" ] ; then echo "Done"; fi
