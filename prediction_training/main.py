...
isLabel = label_features(input_file, labeled_file, True)
...
joblib.dump(scalerX, output_file + '.scl.pkl', compress=False)
...
# save model
clf.save(output_file + '.mdl.pkl')
clf.save(output_file + '.h5')
model_json = clf.to_json()
with open(output_file + '.json', "w") as json_file:
    json_file.write(model_json)
clf.save_weights(output_file + '.h5')
...
error_list, mean = print_error(Ypredicted, Ytarget, range(train_num,total_num), True)
#print zip(Ypredicted[train_num: total_num], Ytarget[train_num: total_num])[:10]
#training error
print "Training error rate:"
error_list, tmp = print_error(Ypredicted, Ytarget, range(0,train_num), False)

#for i in zip(Ypredicted, Ytarget):
#    print i
